...
| `scripts/04-event-management/resend-season-tickets-for-event.js` | Resend season tickets for specific subscription orders. | `node scripts/04-event-management/resend-season-tickets-for-event.js --event=<slug> --order=<orderId[,orderId2]> [--dry-run]` | — |
...
| `scripts/04-event-management/import-invitations-with-qr.js` | Import external invitations with pre-assigned QR codes for scanning. | `node scripts/04-event-management/import-invitations-with-qr.js <invitations.csv> [--event=<slug>] [--status=paid] [--commit] [--force] [--no-finalize]` | — |
