...
| `scripts/04-event-management/import-orders.js` | Import or replay event orders from a CSV export. | `node scripts/04-event-management/import-orders.js <orders.csv> [--status=paid] [--commit] [--force] [--sendEmail]` | `data_references/csv/event-orders.template.csv` |
