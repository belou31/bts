...
| `scripts/04-event-management/send-season-tickets-for-event-from-file.js` | Generate/send season tickets (PDF) for an event based on the season subscriber base. | `node scripts/04-event-management/send-season-tickets-for-event-from-file.js --event=<slug> [--limit=200] [--dry-run]` | — |
| `scripts/04-event-management/resend-season-tickets-for-event-from-orders.js` | Resend season tickets for specific subscription orders. | `node scripts/04-event-management/resend-season-tickets-for-event-from-orders.js --event=<slug> --order=<orderId[,orderId2]> [--dry-run]` | — |
| `scripts/04-event-management/send-event-tickets-from-file.js` | Resend event tickets for orders listed in a CSV file. | `node scripts/04-event-management/send-event-tickets-from-file.js --event=<slug> --file=<orders.csv> [--status=paid] [--dry-run]` | `data/templates/csv/orders-resend.template.csv` |
| `scripts/04-event-management/resend-event-tickets-from-orders.js` | Resend event tickets for specific order IDs. | `node scripts/04-event-management/resend-event-tickets-from-orders.js --event=<slug> --order=<orderId[,orderId2]> [--status=paid] [--dry-run]` | — |
