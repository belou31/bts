"""
Shared logging helpers for LibreOffice macros.

Each macro can wrap its execution with ``capture_logs`` to duplicate stdout/stderr
into a timestamped file under ``/tmp/bts`` (override via ``BTS_LOG_DIR``). The
current log file path is exposed via ``current_log_path`` so UIs can surface it
in notifications.
"""
