#!/usr/bin/env node
/**
 * Import zone tariffs (list or matrix CSV).
 *
 * Usage:
 *   node scripts/03-event-management/pricing/import-zone-tariffs.js <seasonCode> <venueSlug> <csvPath>
 *     [--format=list|matrix] [--delimiter=,|;]
 *
 * Supported formats:
 *   LIST (legacy)  : zoneKey,tariffCode,priceCents|priceEuro
 *   MATRIX (modern): first column tariffCode, subsequent columns zone keys
 *
 * Environment:
 ...
 *   - data/templates/csv/zone-tariffs.template.csv
 *
 * Notes:
 *   - Automatically detects delimiter (comma or semicolon) unless overridden.
 *   - Accepts price cells expressed in euros ("180,00") or cents ("18000").
 */
...
if (!seasonCode || !venueSlug || !csvPath) {
  die('Usage: node scripts/03-event-management/pricing/import-zone-tariffs.js <seasonCode> <venueSlug> <csvPath> [--format=list|matrix] [--delimiter=,|;]');
}
...
(async () => {
  ...
  if (!mongoUri) die('Missing MONGO_URI/MONGODB_URI in .env');
  const connectOpts = {};
  ...
  if (!fs.existsSync(full)) die(`CSV not found: ${csvPath} (cherché aussi dans data/inputs)`);
