...
(async () => {
  ...
  const outPath = outArg || `zone-tariffs-matrix-${seasonCode}-${venueSlug}.csv`;
  const resolvedOut = path.isAbsolute(outPath) ? outPath : path.join(OUTPUT_DIR, outPath);
  ...
  console.log(`Exporté en ${resolvedOut} (tariffs=${tariffs.length}, zones=${zones.length})`);
