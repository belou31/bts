 ...
 *
 * Output:
 *   - Detailed CSV listing missing seats and whether they exist in the SeatCatalog.
 *   - Optional grouped CSV by email summarising missing/unknown seats.
 */
...
(async () => {
  ...
  if (grouped) {
    ...
    const groupedPath = path.isAbsolute(grouped) ? grouped : path.join(OUTPUT_DIR, grouped);
