#!/usr/bin/env node
/**
 * Instantiate seats for a season
 *
 * Looks up seat templates from SeatCatalog (or legacy collections) and
 * creates season-specific Seat documents if they do not exist yet.
 *
 * Usage:
 *   node scripts/01-initialization/venues/instantiate-seats-for-season.js <seasonCode> <venueSlug>
 *
 * Environment:
 *   - MONGO_URI: Mongo database connection (required)
 *
 * Templates:
...
(async () => {
  ...
  if (!seasonCode || !venueSlug) {
    console.error('Usage: node scripts/01-initialization/venues/instantiate-seats-for-season.js <seasonCode> <venueSlug>');
    process.exit(1);
  }
