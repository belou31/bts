...
export async function sendMail({ to, subject, html, text, template, context }) {
  // Si pas de HTML et qu'un template est fourni, on le rend ici
  if (!html && template) {
    try {
      html = await renderEmailTemplate(template, context || {});
    } catch (e) {
      console.warn('[mailer] renderEmailTemplate failed:', e.message || e);
    }
  }
