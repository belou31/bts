...
2. **Spreadsheet-bound project** (per sheet).  
  ...
  - Create a single script file with the content of `scripts_google/btsMenu.gs` (auto-detects the first attached library exposing `createMenu`).
...
Start from the example `scripts_libreoffice/BTS_Config.template.csv` if you need a reference table.
