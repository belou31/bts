...
try:
    from automation_client import ensure_env_loaded as _ensure_env_loaded
    from automation_client import capture_logs
    ...
    repo_root = module_dir.parent.parent  # scripts_desktop/libreoffice/'s grandparent
    for candidate in (module_dir, repo_root):
        if str(candidate) not in sys.path:
            sys.path.append(str(candidate))
    from automation_client import ensure_env_loaded as _ensure_env_loaded  # type: ignore
    from automation_client import capture_logs  # type: ignore
...
def _make_handler(name: str, title: str, command: str):
    ...
    def handler() -> None:
        with capture_logs(name):
            ...
            notify(f"{title}\n\nRun from BTS workspace:\n{command}", document=XSCRIPTCONTEXT.getDocument())
